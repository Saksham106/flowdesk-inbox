...
describe('DELETE /api/knowledge-documents/[id]', () => {
  ...
  it('returns 404 when document belongs to a different tenant', async () => {
    mockSession = { user: { id: 'user1', tenantId: 'tenant-A' } }
    mockFindUnique.mockResolvedValue({ id: 'kd1', tenantId: 'tenant-B' })
  ...
  it('deletes a document that belongs to the session tenant', async () => {
    mockSession = { user: { id: 'user1', tenantId: 'tenant-A' } }
    mockFindUnique.mockResolvedValue({ id: 'kd1', tenantId: 'tenant-A' })
    mockDelete.mockResolvedValue({ id: 'kd1', tenantId: 'tenant-A' })
    mockAuditCreate.mockResolvedValue({})

    const res = await DELETE(makeReq() as never, { params: { id: 'kd1' } })

    expect(res.status).toBe(200)
    expect(mockFindUnique).toHaveBeenCalledWith({ where: { id: 'kd1' } })
    expect(mockDelete).toHaveBeenCalledOnce()
    expect(mockDelete.mock.calls[0][0].where).toMatchObject({ id: 'kd1' })
