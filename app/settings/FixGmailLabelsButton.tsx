...
export default function FixGmailLabelsButton({
  ...
  async function handleClick() {
    ...
    setResult(null)
    try {
      const res = await fetch(`/api/connectors/${provider}/relabel`, { method: "POST" })
    ...
    <div className="rounded-lg border border-slate-100 bg-slate-50 px-4 py-3">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          <p className="text-sm font-medium text-slate-700">Fix {providerLabel} labels</p>
          <p className="mt-0.5 text-xs text-slate-500">
            Recolors your FlowDesk labels and re-applies them to your existing emails.
            Use this if you connected {providerLabel} a while ago and your labels look uncolored
