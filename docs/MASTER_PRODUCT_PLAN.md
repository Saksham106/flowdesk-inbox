...
| 2026-07-13 | Bounded all operational bookkeeping storage: retention cron for receipt tables (AuditLog 30d, AiUsageEvent 90d, push/sync events 30d — PR #163) and stopped writing no-op receipts entirely (cache-hit usage events, unchanged-state audit rows, Outlook full-row rewrites — PR #164). See `docs/storage-and-capacity.md`. | The 2026-07-12 outage was receipt tables (not mail) filling the Postgres volume; post-fix, ~60% of the DB was still bookkeeping and ~80% of the steady-state write rate recorded that nothing happened. With both layers, 5GB Hobby ≈ 300–500 users. |
| 2026-07-13 | Mail retention policy decided as 2 years but deliberately NOT implemented; likewise deferred: Gmail base64-image externalization, index drops, change-driven reconcile. Rationale + revisit triggers in `docs/storage-and-capacity.md`. | Each would delete or destabilize user-visible data to solve a problem that doesn't exist at 7MB of messages; mail-body work also couples rendering to live provider tokens while `invalid_grant` token deaths remain a recurring failure mode. |
