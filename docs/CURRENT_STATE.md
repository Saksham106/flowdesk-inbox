...
## Known-broken (Phase 1 top priority)

These are trust-breaking core-loop failures, not minor edge cases. They make FlowDesk feel broken and are the first thing to fix (see `docs/TODO.md` → Phase 1 and `docs/product-direction.md` → Roadmap). The code paths described elsewhere in this document exist and are unit-tested, but the *end-to-end* loop does not hold up in practice for these two cases:

- **Gmail labels are created but never applied to threads.** `ensureFlowDeskLabels` creates the label shells on connect, but *applying* a label to a thread requires two background steps to actually run: (1) classification decides the label, and (2) the `gmail-writeback` cron drains the queued `apply_labels` job. If those crons are not scheduled/running (they are on the Ops checklist as "must be scheduled or the pipeline is dead"), the user sees empty labels and nothing gets tagged. Fix direction: make label projection a reliable consequence of sync/classification (inline, cron as backup) and surface queue/cron health so a stall is visible.
- **"Mark done" (and other explicit state changes) do not stick on refresh.** The write persists correctly (`workflow-status` route sets `userState:"done"`, `status:closed`), but the home view *recomputes* each conversation's priority every render in `analyzeConversationForCommandCenter` (`lib/agent/command-center.ts`), and the checks run in an order where a draft-ready or freshly re-classified state is evaluated *before* the explicit user "done". Anything that re-creates a draft or re-runs classification resurfaces the item. Fix direction: the explicit user decision must be the highest-priority signal, and the dashboard should trust persisted state rather than re-deriving priority on every render.
