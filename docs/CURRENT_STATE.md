...
Last updated: 2026-06-16 (email iframe rendering: centering fix, double-scrollbar fix, ResizeObserver loop fix; sync idempotency, user override preservation, read state, action-email metadata; Home card UX, right rail cleanup, summary HTML fix; Phase 1 gaps + Phase 2 concierge templates; inbox UX polish; AI cost controls; layout centering, hover row actions, composer redesign, right rail simplification)
