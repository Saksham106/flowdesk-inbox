...
- [ ] Gmail writeback cron hardening (do before/with Phase C): (1) failed jobs retry hot — the catch block never bumps `nextAttemptAt` or fails the job out after max attempts (reuse `nextWritebackAttemptDate` / `GMAIL_WRITEBACK_MAX_ATTEMPTS` already in `lib/google.ts`); (2) no atomic claim — overlapping cron runs can double-process jobs, e.g. double-create Gmail drafts (copy the `updateMany` claim pattern from `lib/agent/job-executor.ts`); (3) empty label sets skip writeback entirely, so stale FlowDesk labels on a thread are never removed when a conversation transitions to a no-label state.
