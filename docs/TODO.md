...
- [x] Drop `FlowDesk/Handle First` from Gmail label projection — Handle First is a dashboard ranking, not a stable per-thread Gmail state, so it is no longer part of the canonical `FlowDesk/*` Gmail label vocabulary.
